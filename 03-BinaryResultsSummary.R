...
for(w in 1:length(w_vec)){
  ...
  for(t in 1:length(mu_c_tab)){
    part1 <- decision %>% filter(Method %in% c("MAP","rMAP", "Alt1", "Alt2") & near(TrueCtrlMean, mu_c_tab[t])) %>% select(Prop)
    part1 <- round(part1, 3)
    
    tt1 <- median_est %>% filter(near(TrueCtrlMean, mu_c_tab[t]))  %>%  group_by(TrueCtrlMean) %>% 
