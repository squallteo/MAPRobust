...
for(m in 1:length(muvec_c)){
  ...
  results = 
    foreach(icount(nsim), .combine = rbind,.packages = c("RBesT","R2jags")) %dopar% {
      ...
      #model averaging
      post_c_rmix = (1 - w_r) * post_c_rmix_b + w_r * post_c_rmix_nb
  ...
  ttt = data.frame(Prop = colMeans(results), Method = c("rMix", "FullHier", "rMAP"), TrueCtrlMean = muvec_c[m])
